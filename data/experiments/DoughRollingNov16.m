...
% d = './';
% Series #
subdird = strcat(dd,'series_3_good/');

filesd = dir(strcat(subdird,'*.txt'));
dough = zeros(length(filesd),2);
dough_d1 = zeros(length(filesd),2);
for ii=1:length(filesd)
    filename = strcat(subdird,filesd(ii,1).name);    
    
    fileID = fopen(filename);
%     {"Dough_D1": [239, 151], "Dough_D2": [506, 145], "Dough_Center": [378, 230], "ID": 0}
    C = textscan(fileID,'%s[%d,%d],%s[%d,%d],%s[%d,%d],%s%d');
    fclose(fileID);
    dough(C{11}+1,:) = [C{8} C{9}];
...
for j=1:length(seq)
    ...
    dough_seq = dough(dough_start:dough_end,:);
    dough_d1_seq = dough_d1(dough_start:dough_end,:);
    ...
    for i=1:length(TrajSeg)
        plot3(Traj(1,SegPoints(i):SegPoints(i+1)),Traj(2,SegPoints(i):SegPoints(i+1)),Traj(3,SegPoints(i):SegPoints(i+1)),'Color', color(TrajSeg(i,1),:),'LineWidth', 3);
        hold on
...
%     drawframe(Camera_Origin_on_Table,0.03)
