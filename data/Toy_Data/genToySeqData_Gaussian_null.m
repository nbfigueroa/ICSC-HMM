...
for i = 1:N   
    ...
    for t = 1:Ti
       ...
       X(:,t) = mvnrnd( Px.Mu(zcur,:),  Px.Sigma(:,:,zcur) );
        ...
        dec_var = 5;
        decision = poissrnd(dec_var);
        if decision>dec_var
            [num id]=max(abs(rand(4,1)));
            null_start = floor(length(X)*id/5);
            null_length = min(Ti/20,floor(rand*50));
            Xnew = X;
            Xnew(:,null_start:(null_start-1)+null_length) =  repmat(X(:,null_start),1,null_length);
            X = Xnew;   
        end           
