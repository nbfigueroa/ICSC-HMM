...
% d = './';
% Series #
subdir = strcat(d,'datos/');

files = dir(strcat(subdir,'*person1_KUKA.txt'));
label_files = dir(strcat(subdir,'*datos_labels.txt'));
data = {};
for ii=1:length(label_files)
    filename = strcat(subdir,files(ii,1).name);    
    filename_labels = strcat(subdir,label_files(ii,1).name);    
    raw_data{ii,1} = textread(filename);    
    labels{ii,1} = textread(filename_labels,'','delimiter',',');
end

data_length = size(raw_data{ii,1},2);

load('./datos/full_rolling_demo_tool_manip.mat')
...
for jj=1:size(raw_data,1)
    poses = [];
%     meas = raw_data{jj};
%     % Frame idx wrt. Kinect
%     Frame_idx{jj,1} = [meas(:,1)']; 
%     
% %     Timestamp
%     TimeStamp{jj,1} = [meas(:,2)']; 
%     
%     EE CART POSITION in Base Frame
%     EE_CART_Pn{jj,1} = [meas(:,3:5)'];
    EE_CART_Pn{jj,1} = pos_est;
    
%     EE CART ORIENTATION in Base Frame
%     EE_CART_On{jj,1} = [meas(:,6:9)'];      
    EE_CART_On{jj,1} = ori_est;  
    
%     EE FT
%     EE_FTn{jj,1} = [meas(:,10:15)'];             
    EE_FTn{jj,1} = cat(1,force_est,torque_est);             
end
...
subplot(4,1,1)
plot(EE_CART_Pn{n,1}','--','Linewidth',2);
legend('x','y','z')

subplot(4,1,2)
plot(EE_CART_On{n,1}','--','Linewidth',2);
legend('qi','qj','qk','qw')

subplot(4,1,3)
plot(EE_FTn{n,1}(1:3,1:end)','--','Linewidth',2);
legend('fx','fy','fz')

subplot(4,1,4)
plot(EE_FTn{n,1}(4:end,1:end)','--','Linewidth',2);
legend('tx','ty','tz')
...
for jj=1:length(EE_CART_Pn)
    P_tmp = EE_CART_Pn{jj,1};
    P_sample = P_tmp(:,1:t:end);
    
    O_tmp = EE_CART_On{jj,1};
    O_sample = O_tmp(:,1:t:end);
    
    FT_tmp = EE_FTn{jj,1};
    FT_sample = FT_tmp(:,1:t:end);
...
Xn = [];
for ii=1:length(EE_FTnP)       
    
    % Check Quaternion    
    Q_check = checkRotations(EE_CART_On{ii,1})*0.25;    
    
    %From pos + or (quaternion)
    X = cat(1,EE_CART_Pn{ii,1},Q_check);      
    
    %From ee_ft
    FT = cat(1,EE_FTn{ii,1}(1:3,:)/10,EE_FTn{ii,1}(4:6,:)*0.25);          
...
subplot(4,1,1)
plot(Xn{n,1}(1:3,:)','--','Linewidth',2);
legend('x','y','z')

subplot(4,1,2)
plot(Xn{n,1}(4:7,:)','--','Linewidth',2);
legend('qi','qj','qk','qw')

subplot(4,1,3)
plot(Xn{n,1}(8:10,:)','--','Linewidth',2);
legend('fx','fy','fz')

subplot(4,1,4)
plot(Xn{n,1}(11:13,:)','--','Linewidth',2);
legend('tx','ty','tz')
...
subplot(4,1,1)
plot(Xn_sm{n,1}(1:3,:)','--','Linewidth',2);
legend('x','y','z')

subplot(4,1,2)
plot(Xn_sm{n,1}(4:7,:)','--','Linewidth',2);
legend('qi','qj','qk','qw')

subplot(4,1,3)
plot(Xn_sm{n,1}(8:10,:)','--','Linewidth',2);
legend('fx','fy','fz')

subplot(4,1,4)
plot(Xn_sm{n,1}(11:13,:)','--','Linewidth',2);
legend('tx','ty','tz')
...
for j=1:3:length(Xn)
    X = Xn_sm{j,1};    
    plot3(X(1,:), X(2,:), X(3,:),'Color',[rand rand rand],'LineWidth', 2);
    hold on 
end
