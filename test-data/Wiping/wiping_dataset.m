...
%         proc_data{2}.X(1,:)    = proc_data{2}.X(1,:) - 0.15 ;
%         proc_data{2}.X(2,:)    = proc_data{2}.X(2,:) - 0.105 ;
%         proc_data{2}.X(3,:)    = proc_data{2}.X(3,:) + 0.015 ;
        ...
        switch variables
            case 0 
                weights = [ones(3,1); 1/2*ones(4,1); 2*ones(3,1); 1/50*ones(3,1); 1/5*ones(3,1)];
                data_legends = {'$x_1$','$x_2$','$x_3$', '$q_i$','$q_j$','$q_k$','$q_w$', ...
                                '$\dot{x}_1$','$\dot{x}_2$','$\dot{x}_3$', ...
                                '$f_x$','$f_y$','$f_z$','$\tau_x$','$\tau_y$','$\tau_z$'};
...
for ii=1:N
    switch variables
        case 0
            Data{ii}  = [proc_data{ii}.X(1:7,:); proc_data{ii}.X_dot(1:3,:); proc_data{ii}.X(8:end,:)];
        case 1
