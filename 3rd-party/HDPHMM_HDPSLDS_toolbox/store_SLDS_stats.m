% function S = store_SLDS_stats(S,n,settings,stateSeq_n,internal_data_struct_n,dist_struct_n,theta_n,hyperparams_n)   
% Store statistics into structure S and save if rem(n,saveEvery) = 0.
% For the SLDS, this includes the continuous state sequence.

function S = store_SLDS_stats(S,n,settings,stateSeq_n,internal_data_struct_n,dist_struct_n,theta_n,hyperparams_n)   

% If we are at a storing iteration:
if rem(n,settings.storeEvery)==0 & n>=settings.saveMin
    % And if we are at a state-sequence storing iteration:
    if rem(n,settings.storeStateSeqEvery)==0
        % Store all sampled state and mode sequences:
        for ii=1:length(stateSeq_n)
            stateSeq_n(ii).state = internal_data_struct_n(ii).obs;
            S.stateSeq(S.n,ii) = stateSeq_n(ii);
        end
        % Increment counter for the state-sequence store variable:
        S.n = S.n + 1;
    end
