...
% Toy Dataset
[Segm_results Total_feats] = plotSegDataNadia(data, bestGauPsi, 1:data.N, 'Best recovered state sequence');
...
% Compute Similarity with Probabilities
tau = 10;
spcm = ComputeSPCMfunctionProb(rec_sigmas, tau);  
figure('Color', [1 1 1]);        
subplot(2,1,1)
imagesc(spcm(:,:,1))
title('Raw SPCM Confusion Matrix')
colormap(copper)
colormap(pink)
colorbar 
