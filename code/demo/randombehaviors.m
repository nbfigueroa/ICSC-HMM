...
%         fprintf('inside if');
    ...
    % Set behaviors for Toy experiment "Time-base invariants"
%     if length(behaviorsID)==4 && notset==0
%         behaviorsID = [2 1 3 2];
%         notset=1;
%     end
    ...
    %To reproduce same trajectories
%     behaviorsID = Ztrue{i,1};
%     Tint = Tn{i,1};
    ...
    for k=1:size(behaviorsID,2)
        ...
        Xik = generateBehaviors(behaviorsID(k),Tint(k));
%         Xik = generateBehaviorsR(behaviorsID(k),Tint(k));
        ...
        % Smoothen segment transitions
        if false
        smooth_int = 50;
        lastSeg = X{i,k-1};
        currSeg = Xcurr;
        transition = [lastSeg(1:3,end-smooth_int-1:end) currSeg(1:3,1:smooth_int)];
        transition_smooth = [smooth(transition(1,:))'; smooth(transition(2,:))'; smooth(transition(3,:))'];
        transition_last = transition_smooth(:,1:smooth_int+1);
        transition_curr = transition_smooth(:,smooth_int+2:(smooth_int+1)*2);
        lastSeg(1:3,end-smooth_int:end) = transition_last;
        currSeg(1:3,1:smooth_int+1) = transition_curr;        
        X{i,k-1} = lastSeg;
        Xcurr = currSeg;
        end
          ...
          for ii=1:length(Xall)
              ...
              if ii==length(Xall)
                  ...
                  if ii==1
                      ...
                      B = cross(N,T);
                      R = [T N B];
                      q = quaternion(R, true)*scale;
                  else
                      
                      ab = Xall(1:3,ii+1) - Xall(1:3,ii);
                      T = abs(ab/norm(ab));
                      ac = Xall(1:3,ii+1) - Xall(1:3,ii-1);
                      T2 = abs(ac/norm(ac));
                      SN = cross(ac,ab);
                      B = cross(T,SN);
                      R = [T B SN];
...
%                 Xallrel(ii,jj) = Xallrel(ii,jj) + Xallrel(ii,jj)*(Nstd*randn(1,1)); 
            end
    end
    XnNoise{i,1}=Xalln;
    XnNoiseO{i,1}=XallnO;
%     XrelNoise{i,1}=Xallrel;
end
...
data = Robotdata; % keeping odata around lets debug with before/after
