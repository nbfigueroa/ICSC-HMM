...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Construct Hierarchical Gaussian Models with Binary Tree Struct %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
root.name = 'Behaviors';
root.idx = 0;
root.Sigma = eye(size(behavs_theta4{1}));

clear t
t = tree(root);
for i=1:length(behavs_theta4)
    name = strcat('Theta_',num2str(i));
    node.name = name;
    node.idx = i; %bphmm idx
    node.Sigma = behavs_theta4{i};
    t  = t.addnode(1,node);
end
...
figname = strcat('Iter: ',num2str(ii-1));
title(figname)
...
% for ii=1:2
    ...
    [min_spcm min_spcm_id] = min(spcm_pairs(:,3));
...
%             new_node_idx = t.get(xi.node_ids(end)).idx + 1;
            ...
            for j=1:2
                ...
                new_spcm = ComputeSPCMfunction(new_comp,0);
