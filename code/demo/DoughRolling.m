...
%% Convert to 12D for Segmentation
Xn = [];
figure
for ii=1:length(EE_Hn)    
    %From pos + or
...
%     FT = cat(1,EE_FTn{ii,1}(1:3,:),EE_FTn{ii,1}(4:6,:));

    
%     Xn{ii,1} = preprocessMocapData(X_tmp,7,2); 
    X_tmp = cat(1,X,FT);  
    Xn{ii,1} = X_tmp;
...
o = [556 14]*2 
Tc_ext = [ -73.512161 	 -481.637259 	 1617.853582 ];
...
%     subplot(fig_row,fig_col,j)
    ...
    Camera_Origin = Chess_Origin*Chess_Origin_Ext^-1;
